...
def detect_vehicles(cam_id, video_file):
    ...
    vdrop = tdf_drop.loc[tdf_drop["fn"] < 2][
        "v_id"
    ].values.tolist()  # minimum of 2 instances filter
    idf.drop(idf.loc[idf["v_id"].isin(vdrop + [np.nan])].index, inplace=True)
    idf.reset_index(drop=True, inplace=True)

    # Determine turns/direction of movement of each vehicle
    ...
    tvdf = vdf.drop(columns=dcols).merge(
        idf.drop(columns=["cls_id"]), on=["v_id", "fn"]
    )
    ...
    cluster_map = (
        cluster_centers[["cluster", "Turns"]].set_index("cluster").to_dict()["Turns"]
    )

    tvdf["Turning Patterns"] = tvdf["cluster"].map(cluster_map)

    cols = ["fn", "cls_id", "vehicle", "features", "x", "y", "w", "h", "Turning Patterns"]
