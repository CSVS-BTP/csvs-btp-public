...
# Permutations of roads between cams
...
def reid_vehicles(cam_ids, videos, output_dir="/app/data/CSVS"):
    ...
    # Intialize output folder directories
    ...
    # Read all csv outputs from detect_vehicles script
    vdfs = []
    ...
    ) # set datatype to torch and move to GPU if available 
    avdf["features"] = avdf["features"].apply(lambda x: x / x.norm()) # Normalise features

    # Find cosine similarity between vehicles for each class across cams for each turn/direction
    ...
    for i in range(len(cam_ids)):
        for j in range(i + 1, len(cam_ids)):
            ...
            pairs = pair_buckets_with_reuse(df1_tp, df2_tp) # permute cam and turn
            dfs_dict = {}
            for tp_i in range(len(df1_tp)):
                for tp_j in range(len(df2_tp)):
                    ...
                    dfm["similarity"] = dfm.apply(
                        lambda x: torch.nn.functional.cosine_similarity(
                            x["features_1"], x["features_2"], dim=0
                        ).item(),
                        axis=1,
                    ).round(2)
                    dfm.drop(columns=["features_1", "features_2"], inplace=True)
                    dfm = dfm.loc[dfm["similarity"] > 0.2].copy() # Threshold for minimum similarity
                    dfm.sort_values(
                        by=["similarity", "v_id_1", "v_id_2"],
                    ...
                    for idx, row in dfm.iterrows():
                        ...
                        v1t = row["v_idt_1"]
                        v2t = row["v_idt_2"]
                        if v1 not in v1s and v2 not in v2s and abs(v1t - v2t) < 30: # Max distance between vehicles being matched
                            v1s.append(v1)
                            v2s.append(v2)
                            indices.append(idx)
                    dfm = dfm.loc[indices].copy()
                    dfm.sort_values(
                        by=["v_id_1", "v_id_2"], ascending=[True, True], inplace=True
                    )
                    dfm.reset_index(drop=True, inplace=True)
                    dfs_dict[(df1_tp[tp_i], df2_tp[tp_j])] = dfm
            # Find a match of cam and turn with max matches
            max_match = 0
            max_pair = None
            for pair in pairs:
                matched = 0
    ...
    # Final dataframes for each cam
    avcols = ["v_id", "cam_id", "fn", "vehicle", "x", "y", "w", "h"]
    fdf_list = []
    for i in range(len(cam_ids)):
        for j in range(i + 1, len(cam_ids)):
            dfa1 = dfa.loc[dfa["cam_id_1"] == cam_ids[i]][
                ["v_id", "v_id_1", "cam_id_1"]
            ]
            dfa2 = dfa.loc[dfa["cam_id_2"] == cam_ids[j]][
                ["v_id", "v_id_2", "cam_id_2"]
            ]
            fdf1 = dfa1.merge(
                avdf[avcols],
                how="left",
                left_on=["v_id_1", "cam_id_1"],
                right_on=["v_id", "cam_id"],
            )
    ...
    # Annote images of matched vehicles
    ...
    # Make matrix of re-identified vehicles
    rdf_dict = {}
    matrix_dict = {}
    for vehicle in vehicle_list:
        ...
        if fdfv.shape[0] == 0:
            matrix_dict[vehicle] = (
                np.zeros((len(cam_ids), len(cam_ids))).astype(int).tolist()
            )
        else:
